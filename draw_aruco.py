...
def get_aruco_pixels(id=0, dictionary=cv2.aruco.DICT_4X4_1000):
...
def draw_aruco_cv2(px_size=10, id=0, dictionary=cv2.aruco.DICT_4X4_1000, data=None):
    if data is None:
        data = get_aruco_pixels(dictionary=dictionary, id=id)
    frame = cv2.resize(255-data*255, (0, 0), fx=px_size, fy=px_size, interpolation=0)
    return frame

def draw_aruco_polylines(xmin, ymin, px_size, id=0, dictionary=cv2.aruco.DICT_4X4_1000, data=None):
    if data is None:
        data = get_aruco_pixels(dictionary=dictionary, id=id)
    polylines = []
